...
from transformers import AutoProcessor, AutoModel, GenerationConfig, TextIteratorStreamer
...
class StableLMBot:
    def __init__(self):
        ...
        # Deep thinking 采样参数（对应 vLLM 的 deep_think）
        self.generation_config_deep_think = GenerationConfig(
            max_new_tokens=1024,
            do_sample=True,
            temperature=1.0,
            top_p=0.95,
        ...
        # Plain 采样参数（对应 vLLM 的 plain）
        self.generation_config_plain = GenerationConfig(
            max_new_tokens=1024,
            do_sample=False,  # greedy decoding
            temperature=1.0,  # 虽然 do_sample=False，但保持一致
            top_p=1.0,
            top_k=-1,
            num_return_sequences=1,
        )
    ...
    async def generate_response(self, video_path, user_input, use_thinking):
        ...
        # 使用模型的 prepare_inputs_embeds 或 _merge_input_ids_with_image_features
        # 这里根据 VILA 模型的实现来准备输入
        print("Preparing inputs...")
        
        # 选择对应的生成配置
        generation_config = self.generation_config_deep_think if use_thinking else self.generation_config_plain
