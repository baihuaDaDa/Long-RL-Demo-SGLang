...
class StableLMBot:
    def __init__(self):
        ...
        # 由于只有 1 张 H800，这里需要先加载推理引擎（vllm 不允许加载时显存被占用，猜测 sglang 同理）
        self.inference_engine = Engine(
            model_path=os.path.join(model_path, "llm"),
            mem_fraction_static=0.35,  # 显存使用比例
            random_seed=42,
            disable_radix_cache=True,  # 使用 input_embeds 时必须禁用 radix cache
            chunked_prefill_size=20480,  # 增加 chunk size（相当于禁用，也可以设置为 =-1）
        )
    ...
    async def generate_response(self, video_path, user_input, use_thinking):
        ...
        # 将 embeddings 转换为正确的格式
        # SGLang 期望单样本的格式是 List[List[float]] (2D list)
        # 即 [[emb1_dim1, emb1_dim2, ...], [emb2_dim1, emb2_dim2, ...], ...]
        embed_tensor = inputs_embeds.squeeze(0).cpu()  # [seq_len, hidden_dim]
        embed_list = embed_tensor.tolist()  # 转换为 Python 嵌套列表
        
        print(f"Embed list format check: type={type(embed_list)}, len={len(embed_list)}, first_elem_type={type(embed_list[0]) if embed_list else None}")
        
        # request_id = random_uuid()
        req = GenerateReqInput(
            input_embeds=embed_list,  # 传入 2D list: [[float, ...], [float, ...]]
            sampling_params=self.sampling_params_deep_think if use_thinking else self.sampling_params_plain,
            stream=True,
        )
        results_generator = self.inference_engine.tokenizer_manager.generate_request(req, None)
